    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl p-6 shadow-xl w-full max-w-sm border border-slate-100">
        <div className="flex flex-col items-center text-center">
          ...
          <h3 className="text-lg font-bold text-slate-900">Mark as Swapped?</h3>
          <p className="text-sm text-slate-500 mt-2">
            This will permanently mark{' '}
            <span className="font-semibold text-slate-700">"{title}"</span> as
            swapped. This action cannot be undone.
          </p>
        </div>
        <div className="flex gap-3 mt-8">
          ...
          <button
            onClick={onConfirm}
            disabled={isSwapping}
            className="flex-1 px-4 py-2.5 rounded-xl bg-teal-600 text-sm font-semibold text-white hover:bg-teal-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-60"
          >
            {isSwapping ? (
